...
def run_rag(model_provider: str = "openai", model_name: str = "gpt-4o") -> None:
    ...
    # Initialize and populate the vector store if we have PDF documents
    ...
    if pdf_paths:
        ...
        try:
            # Use the embedding model path from config.py
            logger.info(f"Using embedding model from config: {EMBEDDING_MODEL_PATH}")

            # Initialize the vector store with the model path from config
